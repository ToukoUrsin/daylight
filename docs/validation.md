# Validation checkpoint — September 21 Pacific / September 22 UTC, 2026

## Passed

- `npm test`: all 13 meaningful Node tests passed. These cover exact end-of-day deadlines, fractional same-day prerequisite handoffs, zero capacity/release restrictions, no overbooking, dependency order, cycles/missing/duplicate IDs, impossible release chains, malformed/oversized/unversioned replay imports, input bounds, shared random draws, deterministic recomputation, and prototype-like imported task IDs.
- `node --check app.mjs`: syntax passed.
- `npm run build`: dependency-free static output produced in `dist/`.
- Running loopback server: `http://127.0.0.1:4328/` and `engine.mjs` returned HTTP200. Hidden Git path returned HTTP403.
- Direct engine calculation: synthetic likely effort13.5h, available17h; all deadlines51.25% before /93.5% after30% essay scope reduction, using seed37 and1,600 simulations. Delta42.25 percentage points, rounded42.3 in UI. No measured outcomes asserted.
